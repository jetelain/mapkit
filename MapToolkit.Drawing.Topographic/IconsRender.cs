...
namespace Pmad.Cartography.Drawing.Topographic
{
    internal static class IconsRender
    {
        public static IDrawIcon Dot(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(5, 5), (target) =>
            {
                target.DrawCircle(new Vector2D(2.5, 2.5), 2, style);
            });
        }
        ...
        public static IDrawIcon Hospital(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(12, 12), (target) =>
            {
                target.DrawRectangle(new Vector2D(0, 0), new Vector2D(11, 11), border);
                target.DrawPolyline(new[] { new Vector2D(0, 5.5), new Vector2D(11, 5.5) }, cross);
                target.DrawPolyline(new[] { new Vector2D(5.5, 0), new Vector2D(5.5, 11) }, cross);
            });
        }
        ...
        public static IDrawIcon WaterTower(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(13, 13), (target) => target.DrawCircle(new Vector2D(6, 6), 6, style));
        }
        ...
        public static IDrawIcon TechnicalTower(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(13, 13), (target) =>
            {
                target.DrawCircle(new Vector2D(6, 6), 6, style);
                target.DrawPolyline(new[] { new Vector2D(1.76, 1.76), new Vector2D(10.24, 10.24) }, stylea);
                target.DrawPolyline(new[] { new Vector2D(10.24, 1.76), new Vector2D(1.76, 10.24) }, stylea);
            });
        }
        ...
        public static IDrawIcon WindTurbine(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(32, 32), (target) => WindTurbine(target, style, stylea, stylec));
        }

        public static void WindTurbine(IDrawSurface w, IDrawStyle style, IDrawStyle stylea, IDrawStyle stylec)
        {
            var center = new Vector2D(16, 16);
            w.DrawPolyline(
                new[]
                {
                    center + new Vector2D( 0.9961946 * 2.5, -0.0871557 * 2.5),
                    center + new Vector2D( 0.6427876 *  15,  0.7660444 *  15),
                    center + new Vector2D( 0.5000000 *  15,  0.8660254 *  15),
                    center + new Vector2D(-0.4226182 * 2.5,  0.9063077 * 2.5),
                    center + new Vector2D(-0.9848077 *  15,  0.1736481 *  15),
                    center + new Vector2D(-1.0       *  15,  0         *  15),
                    center + new Vector2D(-0.5735764 * 2.5, -0.8191520 * 2.5),
                    center + new Vector2D( 0.3420201 *  15, -0.9396926 *  15),
                    center + new Vector2D( 0.5000000 *  15, -0.8660254 *  15),
                    center + new Vector2D( 0.9961946 * 2.5, -0.0871557 * 2.5)
                }, style);
        ...
        {
            var style = w.AllocateBrushStyle(Color.Black);
            return w.AllocateIcon(new Vector2D(32 * .8, 32 * .8), (target) => LightHouse(target, style));
        }
        ...
        public static IDrawIcon Church(IDrawSurface w)
        {
            ...
            return w.AllocateIcon(new Vector2D(18, 18), (target) => Church(target, pen, fill));
        }

        private static void Church(IDrawSurface target, IDrawStyle pen, IDrawStyle fill)
        {
            target.DrawPolyline(new[] { new Vector2D(9, 0), new Vector2D(9, 9) }, pen);
            target.DrawPolyline(new[] { new Vector2D(5, 4), new Vector2D(13, 4) }, pen);
            target.DrawCircle(new Vector2D(9, 13), 3.5f, fill);
        }
    }
}
